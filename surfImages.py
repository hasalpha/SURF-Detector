...
def generate_histograms(total, descriptions, image_array, k):
    ...
    return k_size, image_histograms


def show_dissimilarity_matrices(images, total, k, k_size, grid):
    # Printing the output in a human-readable format
    print(f'K = {k}% of {total} is {k_size}')
    print('Dissimilarity Matrix')
    print_row = "{:>20}" * (len(images) + 1)
    print(print_row.format("", *images))
    for team, row in zip(images, grid):
        print(print_row.format(team, *row), end="\n\n")


def get_keypoints_descriptors(images):
    ...
    for image in images:
        ...
        # Sum total keypoints
        total += len(kp)
    return total, descriptions, image_array

# Return normalized histogram as output
def normalize_histogram(histrogram):
    min = np.min(histrogram)
    max = np.max(histrogram)
    return [(h - min) / (max-min) for h in histrogram]

def compare_histograms(normalized_histogram, grid):
...
def task2(images):
    ...
    # Get total keypoints, individual keypoints and descriptors
    total, descriptions, image_array = get_keypoints_descriptors(images)
    ...
    for k in k_sizes_array:
        ...
        k_size, image_histograms = generate_histograms(
            total, descriptions, image_array, k)

        # Normalize the obtained histogram
        normalized_histogram = normalize_histogram(np.array(image_histograms))
        grid = []

        # Compare image_histograms using Chi Square comparison method
        compare_histograms(normalized_histogram, grid)

        show_dissimilarity_matrices(images, total, k, k_size, grid)
